  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-2xl px-6 py-10">
        <h1 className="text-2xl font-semibold text-black dark:text-zinc-50">{data.name}</h1>
            ...
            {data.seasons.map((season) => (
              ...
              >
                ...
                {season.events.length === 0 ? (
                  ...
                  <ul className="mt-4 flex flex-col gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-800/60">
                      ...
                      <li key={event.id}>
                        ...
                        <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-sm">
                          {event.sessions.map((session) => (
                            <Link
                              key={session.publicSlug}
                              href={`/r/${session.publicSlug}`}
                              className="underline"
                            >
                              {session.name}
                            </Link>

...
The moment a real Orbits export is available: add it (or an anonymized
version) as a new fixture in `fixtures/orbits/csv/`, add a regression test
pinned to it in `packages/parsers/src/orbits-csv/parse.test.ts`, and update
the provenance table in `fixtures/orbits/csv/README.md`. If it reveals a
wrong assumption above, fix this document too — that's the whole point of
writing it down.
